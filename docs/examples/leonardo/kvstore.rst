...
Any object instance is stored as an :class:`Entity` in this model.
...
as string, in :attr:`Attribute.value`.
...
The pipeline structure looks like that of the :ref:`standard pipeline <le-standard-pipeline>`.
...
The entity-attribute-value model offers extreme flexibility, but at a price.
...
is difficult; and interaction with front-ends like Business Intelligence tools, which could easily talk
to a regular relational database, is very difficult.
...
In fact, there are extremely large multi-tenant products that use such models for all their data.
